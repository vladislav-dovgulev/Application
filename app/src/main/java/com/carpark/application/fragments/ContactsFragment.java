...
/**
 * Фрагмент, реализующий отображение контактных данных.
 */
public class ContactsFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        TextView textView = (TextView) this.getView().findViewById(R.id.contacts_text);
        Cursor cursor = database.GetQuery("SELECT * FROM params");
        try {
            if (cursor.moveToFirst()) {
                textView.setText(cursor.getString(cursor.getColumnIndex("contacts")));
            }
        } finally {
            cursor.close();
        }
